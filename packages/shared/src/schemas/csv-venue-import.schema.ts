...
import { VENUE_LAT_RANGE, VENUE_LNG_RANGE, VENUE_NAME_MAX_LENGTH, VENUE_SLUG_MAX_LENGTH } from "./admin-venue.schema";
...
// packages/shared" rule. Field-level limits (name/slug length, priceRange enum, lat/lng range) are
// pulled from the SAME constants `AdminVenueCreateSchema` uses, so CSV import can never persist a row
// the manual create API would reject — the two schemas cannot drift apart on those limits.
