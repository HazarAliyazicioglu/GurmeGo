...
- **#18 (minor-and-patch, 24 update):** #20 ile aynı kök neden (fastify plugin tipleri) — `@fastify/{helmet,compress,multipart}` npm'de zaten en güncel sürümde, fastify 5.12.x ile uyumlu bir plugin sürümü henüz YAYINLANMAMIŞ. Upstream'de bloklanmış, bizim tarafta yapılacak bir şey yok.
- **#24 (zod v4):** önceden NO-GO (bkz. ELENMİŞ).
- **#21 (Tailwind v4): MERGE EDİLDİ (2026-09-23, PR #21, commit 5482816).** `@tailwindcss/postcss` eklendi, postcss.config.js + globals.css v4 sözdizimine (`@import "tailwindcss"` + `@config`) geçirildi, `autoprefixer` postcss'ten kaldırıldı (v4 dahili Lightning CSS ile hallediyor). Codex review 1 MAJOR buldu (`outline-none`→`outline-hidden`, v4'te forced-colors modunda focus göstergesi kayboluyordu) — düzeltilip 17 dosyada uygulandı, CI'da doğrulandı. 1 MINOR (shadow-sm görsel ölçek farkı) bilinçli ertelendi, PR yorumunda not var — bir sonraki UI geçişinde gözle kontrol edilsin.

## Sıradaki adım
**#27 (TS6) derinlemesine incelendi (2026-09-23), TEK BAŞINA ERTELENDİ:** `moduleResolution=node10` + ambient `@types` taraması (TS6'da kapatıldı, `"types":["jest","node"]` eklenmeli) düzeltilebilir görünüyordu, ama asıl sorun daha derinde — Prisma 5.22'nin ürettiği `Prisma.TransactionClient = Omit<DefaultPrismaClient, ITXClientDenyList>` tipi TS6 altında TÜM delegate property'lerini kaybediyor (`contributionQueue`/`venue`/`venueVersion`/`auditLog`/`user`/`$queryRaw` hepsi "does not exist" hatası veriyor), `denylist` kaynağı doğru tanımlı — TS6'nın generic/`Omit` çözümlemesi Prisma 5.x tipleriyle uyumsuz. Bu, transaction içindeki HER admin serviste (queue/users/venues) ve audit log yazma yolunda (ADR 006) tip güvenliğini kırar; `as any` ile yamanacak bir şey değil. PR #27'ye bulgu yazıldı. **TS6 ve Prisma7 (#25) birlikte ele alınmalı** — sıradaki adım: #25 Prisma7'yi (schema `datasource url`→`prisma.config.ts` migrasyonu + `@prisma/client`'ı da eşzamanlı yükseltme) önce çöz, sonra TS6'yı aynı pakette dene. Sonra #20/22/26 NestJS trio (tüm `@nestjs/*` ailesini birlikte yükselten tek koordineli PR gerekiyor). #21 Tailwind4 tamamlandı, #18 upstream'de bloklu — ikisi de kapalı.

**#25 (Prisma7) incelendi (2026-09-23), ERTELENDİ:** Dependabot PR'ı sadece `prisma` CLI'ı (devDependency) 7'ye yükseltiyor, `@prisma/client`'ı 5.14.0'da bırakıyor — CLI7+Client5 desteklenmeyen kombinasyon, bu haliyle merge edilirse `prisma generate` kırılır (nitekim kırılıyor). Gerçek geçiş: `@prisma/client`'ı da eşzamanlı yükseltmek + `datasource.url`→`prisma.config.ts` migrasyonu + 5→7 iki majör atlama. ORM config mimarisi değişikliği = ADR gerektirir (CLAUDE.md kuralı), ayrı brainstorming→ADR→plan→red-team döngüsü açılmalı, dependabot PR'ı olduğu gibi merge edilemez. PR #25'e bulgu yazıldı.
...
- **Worktree'de `apps/api` typecheck'i bazen aynı TS sürümü/kaynak/config ile bile master'dan FARKLI (yanlış) hata veriyor** (Tailwind PR incelemesinde görüldü — kaynak/config/generated Prisma client birebir aynıydı, temiz reinstall bile düzeltmedi, ama gerçek GitHub CI'da tamamen yeşildi): ELENDİ, KALICI — yerel worktree'de apps/api typecheck hatası görürsen KÖRÜ KÖRÜNE gerçek sanma, önce PR'ı push edip gerçek CI'da doğrula, kök nedeni bulamazsan takılma.
