...
**Dependabot 13 PR'ının (#15-27) triyajı yapıldı (2026-09-22):** #15/#16/#17/#19/#23 CI yeşil + canlı doğrulama ile MERGE EDİLDİ (#16/#17 için kullanıcı `gh auth refresh -s workflow` çalıştırdı). #18 (minor-and-patch grubu, fastify 5.11.3→5.12.5 içeriyor) `apps/api` typecheck'i @fastify/{helmet,compress,multipart} ile kırıyor — ELENDİ, ayrı incelenmeli. Kalan 6 majör bump (#20/22/26 NestJS trio, #21 tailwind v4, #25 prisma v7, #27 typescript v6) bilinçli ERTELENDİ.

**#24 (zod v3→v4) idea-red-team'den NO-GO aldı** (Codex yüksek efor): `AdminVenueUpdateSchema = AdminVenueCreateSchema.partial()` üzerinden `signatureItems: ...default([])` miras alıyor — v4'te `.partial()` default'u SESSİZCE output'a enjekte ediyor (ampirik doğrulandı: `{status:"ARCHIVED"}` → v3 `{status:"ARCHIVED"}`, v4 `{signatureItems:[],status:"ARCHIVED"}`). Salt `status` güncelleyen bir PUT, mevcut imza yemeklerini SESSİZCE SİLER (`venues.repository.ts:447`). Ek: v4 UUID regex'i sıkılaştı (fixture'lar geçersiz olur), string uzunluğu Unicode code point oldu (emoji sınırı değişti). PR #24 ERTELENMİŞ, kod değişikliği yok.

## Sıradaki adım
(a) Kalan 6 majör bump + #18 için ayrı ayrı brainstorming; zod v4 tekrar gündeme gelirse önce `AdminVenueUpdateSchema`'nın `signatureItems` gibi default'lu alanlarını `.partial()` yerine elle `.optional()` ile yeniden tanımlamak ŞART; (b) AK-02/AK-03 açık ürün kararları; (c) gerçek Supabase kurulunca DB rol script'i uygulanacak.
...
- Review/red-team'i tek turda bitirmeyi ummak · CI'nın "yazıldı = çalışıyor" varsayımı · cross-session guard'larda TEK sinyal · Codex çıktısını görmeden "çalışıyor" saymak: ELENDİ, KALICI. Kural: `codex exec … - < dosya` (stdin), bitiş = çıktıda `tokens used` var.
- Majör bağımlılık yükseltmesini "testler yeşil" ile kapatmak: ELENDİ, KALICI (canlı probe gerekir; zod v4 vakası bunun en somut kanıtı — bkz. aşağı).
...
- `gh pr merge --squash` sonrası yerel `master` squash-öncesi commit içeriyorsa "fast-forward yapılamıyor" hatası verir: ELENDİ, KALICI — PR GitHub'da yine de merge olmuştur (`gh pr view --json state,mergedAt` ile doğrula), yerel `master`'ı `git reset --hard origin/master` ile hizala.
...
- Dependabot fastify'ı `5.11.3`→`5.12.5` (PATCH!) yaptığında `@fastify/{helmet,compress,multipart}` ile typecheck kırıldı (PR #18, ELENDİ) — patch sürümler bile plugin tipleriyle kırılabiliyor. zod v4'te `.partial()`, `.default(...)` alanlarını output'a SESSİZCE enjekte ediyor (v3'te omitted alan `undefined` kalırdı): ELENDİ, KALICI — `X.partial()` ile türetilen HER update şemasını, temel şemadaki her `.default(...)` alan için ayrı denetle; "input=output tipi, transform yok, güvenli" çıkarımı YETERSİZ, BAŞARILI parse çıktısının şeklini de canlı test et.
