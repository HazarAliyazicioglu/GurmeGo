...
export function VenueDetail({ venue }: { venue: VenueDetailType }) {
  return (
    <article data-testid="venue-detail" className="relative mx-auto overflow-hidden pb-10 pt-6 sm:pt-10 lg:pb-16">
      <div className="pointer-events-none absolute -right-28 top-6 -z-10 size-80 rounded-full bg-terracottaLight/10 blur-3xl" aria-hidden="true" />

      <header className="border-b border-ink/12 pb-6 sm:pb-8">
        <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
          <p className="text-[0.64rem] font-black uppercase tracking-[0.2em] text-terracottaDeep">
            {CATEGORY_LABELS[venue.category] ?? venue.category.replaceAll("-", " ")}
          </p>
          {venue.isBoutique && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-sand px-2.5 py-1 text-[0.58rem] font-black uppercase tracking-[0.14em] text-brown">
        ...
        <div className="mt-3 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            <h1 className="max-w-[18ch] font-serif text-[2.65rem] font-semibold leading-[0.94] tracking-[-0.055em] text-ink sm:text-6xl lg:text-7xl">{venue.name}</h1>
            <p data-testid="district-name" className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-ink/55">
              <svg viewBox="0 0 20 20" className="size-4 fill-none text-terracotta" aria-hidden="true"><path d="M10 17s5-4.8 5-9a5 5 0 1 0-10 0c0 4.2 5 9 5 9Z" stroke="currentColor" strokeWidth="1.7" /><circle cx="10" cy="8" r="1.6" fill="currentColor" /></svg>
              {venue.district.name}
            </p>
          </div>
      ...
      <div className="grid gap-5 pt-5 sm:gap-6 sm:pt-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,0.65fr)]">
        <div className="space-y-5 sm:space-y-6">
          <section className="relative overflow-hidden rounded-[1.5rem] bg-ink px-5 py-6 text-cream shadow-[0_18px_45px_rgba(32,29,24,0.13)] sm:px-7 sm:py-7">
            <svg viewBox="0 0 120 120" className="absolute -right-5 -top-7 size-36 text-terracottaLight/15" aria-hidden="true"><circle cx="60" cy="60" r="48" fill="none" stroke="currentColor" strokeWidth="12" /><circle cx="60" cy="60" r="22" fill="currentColor" /></svg>
            <div className="relative">
              <p className="flex items-center gap-2 text-[0.63rem] font-black uppercase tracking-[0.2em] text-terracottaLight"><span className="h-px w-6 bg-current" aria-hidden="true" />Neden burada?</p>
              {venue.editorialNote ? (
                ...
                <p className="mt-4 max-w-[42ch] font-serif text-xl leading-snug text-cream/65">Kürasyon ekibinin mekan notu yakında burada.</p>
              )}
              <p className="mt-5 text-[0.62rem] font-black uppercase tracking-[0.17em] text-cream/42">GurmeGo kürasyon ekibi</p>
            </div>
          </section>
          ...
          <section className="rounded-[1.5rem] border border-ink/12 bg-creamLight p-5 sm:p-6">
            <div className="grid gap-5 sm:grid-cols-[8rem_1fr] sm:gap-7">
              <div className="border-b border-ink/10 pb-5 sm:border-b-0 sm:border-r sm:pb-0 sm:pr-7">
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-ink/42">Fiyat aralığı</p>
                <p data-testid="price-range" className="mt-2 font-serif text-3xl font-semibold tracking-[0.08em] text-terracotta">{PRICE_RANGE_LABELS[venue.priceRange]}</p>
                <p className="mt-1 text-xs font-semibold text-ink/45">kişi başı tahmini</p>
              </div>
              <div>
                <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-ink/42">İmza lezzetler</p>
                <ul data-testid="signature-items" className="mt-3 flex flex-wrap gap-2">
                  {venue.signatureItems.map((item) => <li key={item} className="rounded-full border border-terracotta/25 bg-terracotta/[0.08] px-3 py-2 text-sm font-bold text-brown">{item}</li>)}
                </ul>
              </div>
            </div>
          </section>

          <section className="overflow-hidden rounded-[1.5rem] border border-ink/12 bg-sandLight" aria-labelledby="directions-title">
            <div className="relative min-h-36 overflow-hidden p-5 sm:min-h-40 sm:p-6">
              ...
              <div className="absolute -bottom-12 -right-8 size-44 rounded-full border-[24px] border-terracotta/20" aria-hidden="true" />
              <div className="relative flex min-h-24 flex-col justify-between gap-5 sm:flex-row sm:items-end">
                <div>
                  <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-terracottaDeep">Sıradaki durak</p>
          ...
          <section className="rounded-[1.5rem] border border-ink/12 bg-creamLight p-5 sm:p-6" aria-labelledby="venue-location-title">
            <p id="venue-location-title" className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-ink/42">
              Konum ve fotoğraflar
            </p>
            {venue.address && (
              <p data-testid="venue-address" className="mt-3 text-sm font-semibold leading-relaxed text-ink/70">
                {venue.address}
              </p>
            )}
          ...
          {venue.googleRating != null && (
            <a data-testid="google-rating" href={`https://maps.google.com/?q=${encodeURIComponent(venue.name)}`} target="_blank" rel="noreferrer" className="group flex min-h-12 items-center justify-between gap-3 rounded-full border border-ink/12 bg-creamLight px-4 text-sm font-black text-ink transition-colors hover:border-terracotta/50 focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-terracotta focus-visible:ring-offset-2 focus-visible:ring-offset-cream" aria-label={`${venue.googleRating} yıldız, ${venue.googleRatingCount} Google yorumu`}>
              <span className="flex items-center gap-2">
                ...
                <span className="font-medium text-ink/40">{venue.googleRatingCount} Google yorumu</span>
              </span>
              <span className="text-ink/35 transition-transform group-hover:translate-x-0.5" aria-hidden="true">Google'da gör ↗</span>
            </a>
          )}
        </div>
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-[1.5rem] border border-ink/12 bg-creamLight p-5 sm:p-6">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-ink/42">Birlikte karar ver</p>
            <p className="mt-2 text-sm font-medium leading-relaxed text-ink/55">Bu mekanı plan yaptığın kişiye gönder.</p>
          ...
          <section className="rounded-[1.5rem] border border-ink/12 bg-creamLight p-5 sm:p-6">
            <p className="text-[0.62rem] font-black uppercase tracking-[0.18em] text-ink/42">Ziyaret bilgisi</p>
            <p data-testid="opening-hours" className="mt-4 grid grid-cols-2 gap-x-5 gap-y-2.5 text-xs">
              {Object.entries(venue.openingHours).map(([day, hours]) => (
                <span key={day} className="flex items-baseline justify-between gap-2 border-b border-ink/[0.08] pb-2"><span className="font-black text-ink/45">{DAY_LABELS[day.toLowerCase()] ?? day}</span><span className="font-bold tabular-nums text-ink/75">{hours}</span></span>
              ))}
            </p>
            <p data-testid="verified-at" className="mt-4 flex items-center gap-2 text-[0.68rem] font-bold text-ink/42"><span className="grid size-5 place-items-center rounded-full bg-terracotta/12 text-terracottaDeep" aria-hidden="true">✓</span>Son doğrulama: {new Date(venue.verifiedAt).toLocaleDateString("tr-TR")}</p>
          </section>
        </aside>
      </div>
    </article>
