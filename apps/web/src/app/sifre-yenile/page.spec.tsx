...
describe("SifreYenilePage", () => {
  beforeEach(() => {
    push.mockClear();
    updatePassword.mockReset();
  });

  it("says the link is invalid/expired (with a way to request a new one) when there is no recovery session", () => {
  ...
  it("shows a loading state (not the invalid-link error) while the session is still resolving", () => {
    mockUseAuth.mockReturnValue({ user: null, loading: true, updatePassword });
    render(<SifreYenilePage />);
    expect(screen.queryByRole("alert")).toBeNull();
    expect(screen.getByRole("status")).toBeInTheDocument();
  });

  it("updates the password and goes to /favoriler", async () => {
