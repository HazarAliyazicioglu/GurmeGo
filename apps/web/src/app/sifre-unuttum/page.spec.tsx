...
describe("SifreUnuttumPage", () => {
  it("sends the reset link and shows a confirmation naming the address", async () => {
    ...
    fireEvent.change(screen.getByLabelText("E-posta"), { target: { value: "me@x.com" } });
    fireEvent.click(screen.getByRole("button", { name: "Sıfırlama linki gönder" }));
    expect(await screen.findByRole("status")).toHaveTextContent("me@x.com");
    expect(requestPasswordReset).toHaveBeenCalledWith("me@x.com");
  });

  it("shows the (translated) error and keeps the form when the request fails", async () => {
